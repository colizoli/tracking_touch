...
if subject_ID:
    ...
    staircase1 = pd.read_excel(os.path.join(logfile_dir, 'sub-{}_staircase_{}.xlsx'.format(subject_ID, 0)))
    # staircase2 = os.path.join(logfile_dir, '{}_staircase_{}'.format(subject_ID, 1))
    # staircase3 = os.path.join(logfile_dir, '{}_staircase_{}'.format(subject_ID, 2))
    
    touch_intensities = [
        np.array(staircase1['All Intensities'])[-1],    # top 
        # np.array(staircase2['All Intensities'])[-1],  # middle
        # np.array(staircase3['All Intensities'])[-1],  # bottom         
        ]
    ...
    for t in trials:
        ...
        # touch_1 = t
        if touch_mode:
            digitimer_functions.administer_touch(channel=t, this_intensity=touch_intensities[t-1]) # channels are non-zero
        core.wait(t_touch)
        print('Touch1={}, Intensity={}'.format(t, touch_intensities[t-1]))
        ...
        if touch_mode:
            digitimer_functions.administer_touch(channel=feedback, this_intensity=touch_intensities[feedback-1]) # channels are non-zero
        core.wait(t_touch)   
        print('Touch2={}, Intensity={}'.format(feedback, touch_intensities[feedback-1]))
