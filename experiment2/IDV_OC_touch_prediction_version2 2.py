...
\nPlace your hand so it is facing UPWARDS\
...
\nPlace your hand so it is facing DOWNWARDS\
\n\nThere will be 9-10 blocks in total (~5 min each), each with several trials.\
\nIn between the blocks, you can take a break to move your head.\
\n\nWhen you are ready to continue with the next block,\
\nplace your head back in the chin rest and fixate on the dot at the center of the screen.\
...
normal_down_break_text = "Take a short break!\
...
first_down_flipped_break_text = "Take a short break!\
\n\nFROM NOW ON, POSITION YOUR LEFT HAND SO IT IS FACING UPWARDS. \
\n\nAgain the keys are Index/ Middle/ Ring finger: Left/ Down /Right arrow keys.\
\n\nPush any button when you are ready to CONTINUE."
...
flipped_down_break_text = "Take a short break!\
\n\n Keep your left hand facing UPWARDS. \
\n\nAgain the keys are Index/ Middle/ Ring finger: Left/ Down /Right arrow keys.\
\n\nPush any button when you are ready to CONTINUE."
...
if initial_hand == 'up':
    ...
    stim_instr.setText(instr3_down_txt)
    stim_instr.draw()
    win.flip()
    core.wait(0.25)
    event.waitKeys()
...
def present_break(break_text):
    ...
    \nYou have completed {block_counter - 1}/10 blocks")
    stim_instr.draw()
    win.flip()
    core.wait(3)
...
def present_block():
    ...
    for t in trials:
        ...
        DF.loc[trial_num] = [
                ...
                initial_hand,       # initial hand position
...
if initial_hand == 'up':
    ...
    for block in np.arange(MIN_BLOCKS):
        ...
        if block == MIN_BLOCKS - 1:
            ...
            # Show transition break depending on performance
            if get_acc < THRESH_ACC:
                present_break(normal_down_break_text)  # get ready for another unflipped block
                get_acc = present_block()  # present the extra block
                present_break(normal_down_break_text)  # break after extra block
            else:
                flip = 1  # start flipped condition
                present_break(first_down_flipped_break_text)
                for block in np.arange(FLIP_BLOCKS):
                    present_block()
                    present_break(flipped_down_break_text)
        else:
            present_break(normal_down_break_text)
