...
MAX_BLOCKS = 6     # maximum number of blocks to reach desired accuracy level
FLIP_BLOCKS = 5    # number of blocks to administer after flipping the hand
# accuracy
TRESH_ACC = 0.75   # desired accuracy level before flipping the hand
...
\n\n FROM NOW ON, WE WOULD LIKE YOU TO POSITION YOUR LEFT HAND DOWNWARDS. \
\n\nAgain the keys are Index/ Middle/ Ring finger: Left/ Down /Right arrow keys.\
\nRemember to blink as you normally would, but do NOT move your left hand during the experiment.\
\n\nPush any button when you are ready to CONTINUE."
...
if get_acc < TRESH_ACC: # check accuracy try one more block?
    get_acc = present_block() # present blocks
    present_break(normal_break_text)
    
if get_acc >= TRESH_ACC:
    # tell them to flip hand for 5 more blocks
    flip = 1
    break_text = first_flipped_break_text # start with first flipped break text

    # present flipped blocks
    for block in np.arange(FLIP_BLOCKS):
        present_block() # present blocks
        present_break(break_text)
